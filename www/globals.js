...
if (local == true){
	...
	masterurl = "http://wingmanapi.herokuapp.com"
}
...
function logoutUser() {
	$.post(masterurl + "/api/user/logout", {token: logged_user.token}, function(data) {
		if (data.success) {
...
//		$("#miles-p").html(this.total_miles + "###"); later
	...
	function createUser(name, email, password) {
		...
		if (name == name2 && email == email2 && password == password2){
			$.post(masterurl + "/api/user/create", { name: name, email: email, password: password }).done(function(data) {
				if (data.success) {
					//send email
					$.ajax({
						 type: "POST",
	...
	User.prototype.updateFlight = function() {
		...
		$.get(masterurl + "/api/currentdata", {
			...
			$.post(masterurl + "/api/user/update", {
				flight: logged_user.Get("flightnum"),
				token: logged_user.token,
			...
			if (airportsshown == false && data.departfrom != undefined && data.arriveat != undefined){
				...
				document.getElementById('flightbox').innerHTML = "<h1>" + capitalize(logged_user.Get("flightnum").replace(/\+/g, " ")) + "</h1>" + "<h6>" + departairport + " <img id = 'smallplane' src = 'img/angled/90small.png' alt = 'planeicon'> " + arriveairport + "</h6>";
				airportsshown = true;
			}
			//document.getElementById('airports').innerHTML += "<span class = 'boldy'>" + data.departfrom + " to " + data.arriveat + "</span>";   


			//Variable data at different times: 
			...
			$.get(masterurl + "/api/user/pathcoords", {
				flight: logged_user.Get("flightnum"),
				token: logged_user.token
			}, function(data){
...
// INSERT LOADING GIF HERE NEXT TWO POSSIBLE SUBMITS?
	...
	function submitChat()
	{
		...
		$.post(masterurl + "/api/chat/submit", {username: logged_user.email, chatline: chatmsg, token: logged_user.token});
	}

	var chat_calls = 0;
	function getLastTenLines() {
		$.get(masterurl + "/api/chat/chatlines?token=" + logged_user.token, function(data) {
